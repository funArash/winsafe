...
pub trait kernel_Hkey: Handle {
    ...
    fn RegGetValue(
        ...
        validate_retrieved_reg_val(
            unsafe { co::REG::from_raw(raw_data_type1) },
            data_len1,
            unsafe { co::REG::from_raw(raw_data_type2) },
            data_len2,
            buf,
        )
    }
    ...
    fn RegQueryMultipleValues(
        ...
        let mut valents2 = value_names_w
            .iter()
            .map(|value_name_w| {
        ...
        Ok(valents2
            .iter() // first VALENT array is not filled with len/type values
            .map(|v2| unsafe {
                RegistryValue::from_raw(v2.buf_projection(&buf).to_vec(), v2.ve_type)
            })
            .collect::<Vec<_>>())
    }
